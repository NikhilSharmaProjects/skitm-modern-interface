  ...
  useEffect(() => {
    ...
    if (savedEvents) {
      ...
      if (cleanedEvents.length < parsedEvents.length) {
        localStorage.setItem('skitm-events', JSON.stringify(cleanedEvents));
      }
            ...
            <div className="space-y-12">
              ...
              <div className="space-y-8">
                ...
                {upcomingEvents.length === 0 ? (
                  <div className="glassmorphism rounded-xl p-8 text-center">
                    <p className="text-skitm-gray">No upcoming events at the moment. Check back soon!</p>
                  </div>
                ) : (
              ...
              {pastEvents.length > 0 && (
                <div className="space-y-8">
                  <h2 className="text-2xl font-display font-bold text-skitm-navy">Past Events</h2>
                  
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {pastEvents.map((event) => (
                      <div key={event.id} className="glassmorphism rounded-xl overflow-hidden hover:shadow-md transition-shadow opacity-75">
                        {event.image && (
                          <div className="h-48 overflow-hidden">
                            <img 
                              src={event.image} 
